...
/**
 * Sales Order hold. A submitted Sales Order can be put on hold (e.g. a dispute or
 * a credit review) so nothing ships or bills against it until it is resumed. The
 * hold is enforced by a before_submit gate on Delivery Note and Sales Invoice.
 * Pure SQL over sibling tables; Selling imports no other module's services.
 */
@Injectable()
export class SalesOrderHoldService {
  private readonly logger = new Logger(SalesOrderHoldService.name);
  ...
  private async setHold(order: string, hold: boolean, reason: string | null): Promise<void> {
    ...
    const rows = await this.dataSource.query(
      `SELECT ${quoteIdent("docstatus")} AS docstatus FROM ${quoteIdent(tableNameFor("Sales Order"))}
       WHERE ${quoteIdent("name")} = $1`,
      [order],
    );
    if (rows.length === 0) throw new BadRequestException(`Sales Order ${order} not found`);
    if (Number(rows[0].docstatus ?? 0) !== 1) {
      throw new BadRequestException(`Sales Order ${order} must be submitted to ${hold ? "hold" : "resume"}`);
    }
  ...
  async hold(order: string, reason: string): Promise<{ order: string; on_hold: boolean }> {
    ...
    this.logger.log(`Sales Order ${order} put on hold`);
  ...
  async resume(order: string): Promise<{ order: string; on_hold: boolean }> {
    ...
    this.logger.log(`Sales Order ${order} resumed`);
  ...
  private async isHeld(order: string): Promise<boolean> {
    ...
    const row = (
      await this.dataSource.query(
        `SELECT ${quoteIdent("on_hold")} AS on_hold FROM ${quoteIdent(tableNameFor("Sales Order"))}
         WHERE ${quoteIdent("name")} = $1`,
        [order],
      )
    )[0];
  ...
  private async gate(doc: Record<string, unknown>, verb: string): Promise<void> {
    ...
    if (order && (await this.isHeld(order))) {
      throw new BadRequestException(
        `Cannot ${verb} against Sales Order ${order} — it is on hold`,
      );
    }
  }
}
